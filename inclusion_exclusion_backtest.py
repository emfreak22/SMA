...
import pandas as pd
import datetime
START_DATE = datetime.datetime(1998, 8, 1)
...
def get_list_data_updated(date= '01-08-1998', symbol_data= []):
    ...
    for row,data in working_data.iterrows():
        ...
        if data['Description'] == INCLUSION and entry not in symbol_data:
            print(f"📈📈📈📈📈📈📈📈📈📈Inclusion of {data['Symbol']} :📈📈📈📈📈📈📈📈📈📈📈📈📈")
            symbol_data.append(entry)
        else:
            print(f"📉📉📉📉📉📉📉📉📉📉📉Exclusion of {entry['Symbol']} is done 📉📉📉📉📉📉📉📉📉")
...
def get_list_data_v2(date='01-08-1998', symbol_data=None):
    ...
    for _, row_data in working_data.iterrows():
        if row_data['Description'] == INCLUSION:
            ...
            symbol_data = [entry for entry in symbol_data if entry['Symbol'] != row_data['Symbol']]
...
def get_top_stocks_by_volume(combined_df, date, top_n=100):
    ...
    try:
        ...
        top_stocks = (
            combined_df.groupby(level = "Symbol")["Volume"].sum().nlargest(top_n).index.tolist()
        )
        return top_stocks
    except Exception:
        # print("No data today")
...
def check_long_close_condition(stock_data, updated_stock_symbol):
    ...
    if condition1:
        print_red(f" 💀 💀 💀 💀 💀 💀💀 💀Condition 1 met: {stock_data['Symbol']} is delisted, hence selling. 💀 💀 💀 💀 💀 💀 💀 💀 💀 💀 💀 💀")
        DELISTED.append(stock_data)
        return True
    elif condition2:
        print(f"Condition 2 met: Fast_MA ({stock_data['Fast_MA']}) is less than Slow_MA ({stock_data['Slow_MA']}).")
        return True
    else:
        return False
...
def process_trades(combined_df, start_balance, N=N):
    ...
    dates = pd.date_range(start=START_DATE, end=END_DATE)  # Replace this with your actual DatetimeIndex
    for date in dates:
        if isinstance(date, datetime.datetime):
          date = date.strftime("%d-%m-%Y")
        print(date)
        updated_stocks_list = get_list_data_updated(date= date, symbol_data=symbol_data)
        updated_stocks_symbols = [i['Symbol'] + ".NS" for i in updated_stocks_list]
        try:
            today_data =  combined_df.loc[date]
        except Exception:
            # print(f"{date} is a weekend or there is no data..")
            continue
        filtered_data =  today_data[today_data.index.get_level_values('Symbol').isin(updated_stocks_symbols)]
        if len(portfolio) < N:
                    ...
                    if len(portfolio) < N:
                        ...
                        if new_symbols:
                            for new_symbol in new_symbols:
                                if len(portfolio) >= N:
                                    break
                                if new_symbol != portfolio_symbol and new_symbol not in portfolio :
